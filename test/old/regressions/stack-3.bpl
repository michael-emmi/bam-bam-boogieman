...
function $isExternal(p: int) returns (bool) { p < -33134 }
const $GLOBALS_BOTTOM: int;
axiom $GLOBALS_BOTTOM == -366;
...
const {:count 2} unique collision: int;
const unique delay: int;
const unique int_cas: int;
const {:count 7} unique location: int;
const unique main: int;
const unique ti_cas: int;
const unique unique_id: int;
procedure $static_init()
  modifies $M.0, $M.1, $M.2, $M.3, $M.4, $M.5, $M.6, $Alloc, $CurrAddr;
{
...
$bb0:
  ...
  $p1 := $M.0[$p];
  $p2 := $p1;
  $p3 := $pa($pa(location, 0, 56), $p2, 8);
  $M.5[$p3] := p;
  call $p4 := GetPosition(p);
  $p5 := $p4;
  $p6 := $pa($pa(collision, 0, 8), $p5, 4);
  $p7 := $M.6[$p6];
  $p8 := $p7;
  goto $bb1;
$bb1:
  $p9 := $p4;
  $p10 := $pa($pa(collision, 0, 8), $p9, 4);
  call $b := int_cas($p10, $p8, $p1);
  $b11 := $i2b($xor($b2i($b), $b2i(true)));
  goto $bb4, $bb5;
$bb2:
  $p43 := $p4;
  $p44 := $pa($pa(collision, 0, 8), $p43, 4);
  $p45 := $M.6[$p44];
  $p8 := $p45;
  goto $bb1;
$bb3:
...
$bb26:
  ...
  goto $bb24;
$bb27:
  $p29 := $pa($pa($p23, 0, 32), 4, 1);
  $p30 := $M.0[$p29];
  $p31 := $p30;
  $p32 := $pa($pa(p, 0, 32), 4, 1);
  $p33 := $M.0[$p32];
  $p34 := $p33;
  $b35 := ($p31 != $p34);
  goto $bb31, $bb32;
...
$bb30:
  assume {:yield} true;
  $p36 := $p1;
  $p37 := $pa($pa(location, 0, 56), $p36, 8);
  call $b38 := ti_cas($p37, p, 0);
  goto $bb35, $bb36;
$bb31:
  assume $b35;
  goto $bb30;
$bb32:
  ...
  goto $bb24;
$bb33:
  call $b39 := TryCollision(p, $p23, $p8);
  $p40 := $b2p($b39);
  $b41 := ($p40 == 1);
  goto $bb39, $bb40;
$bb34:
...
$bb0:
  ...
  call {:async t2} Push(1);
  call {:async t3} Push(1);
  call {:async t4} x := Pop();
  call {:async t5} Push(1);
  call {:async t6} x := Pop();
  assume {:yield} true;
  $r := 0;
  return;
