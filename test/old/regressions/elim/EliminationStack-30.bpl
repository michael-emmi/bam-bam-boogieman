...
$bb1:
  assume {:yield} true;
  $p24 := him;
  $p25 := $pa($pa(location, 0, 112), $p24, 8);
  call $b26 := ti_cas($p25, q, p);
  goto $bb20, $bb21;
$bb2:
  $p5 := $pa($pa(p, 0, 32), 4, 1);
  $p6 := $M.0[$p5];
...
$bb11:
  $p14 := him;
  $p15 := $pa($pa(location, 0, 112), $p14, 8);
  call $b16 := ti_cas($p15, q, 0);
  goto $bb16, $bb17;
$bb12:
  assume $b13;
  goto $bb10;
$bb13:
  ...
  goto $bb11;
$bb14:
  $p17 := $pa($pa(q, 0, 32), 8, 1);
  $p18 := $pa($pa($p17, 0, 16), 8, 1);
  $p19 := $M.0[$p18];
  $p20 := $pa($pa(p, 0, 32), 8, 1);
  $p21 := $pa($pa($p20, 0, 16), 8, 1);
  $M.0[$p21] := $p19;
  $p22 := $p1;
  $p23 := $pa($pa(location, 0, 112), $p22, 8);
  $M.5[$p23] := 0;
  $b9 := true;
  goto $bb9;
$bb15:
