  ...
  class Main
    ...
    def command_line_options(args)
      ...
      args.each_index do |idx|
        next if idx > 0 && file_switch?(args[idx-1])
        next unless File.extname(args[idx]) == '.bpl'

        # Add the read switch before any solitary Boogie files
        args.insert(idx, Bpl::Reading.switch[:args].first)
      end
    ...
    def main(args)
      ...
      command_line_options(args).parse!(args)
