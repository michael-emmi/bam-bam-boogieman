...
# from lexical definition file "/Users/mje/Code/c2s/lib/z3/model_parser.rex".
#++
  ...
  def scan_evaluate( str )
    ...
    until ss.eos?
      ...
      when nil
        ...
        when (text = ss.scan(/as-array\[k!\d+\]/))
           @rex_tokens.push action { [:WEIRD, nil] }
        ...
        when (text = ss.scan(/[a-zA-Z_.$\#'`~^\\?][\w.$\#'`~^\\?]*@\d*_si_\d+/))
           @rex_tokens.push action { [:WEIRD, nil] }
        ...
        when (text = ss.scan(/\%lbl\%\+si_fcall_\d+/))
           @rex_tokens.push action { [:LABEL, {id: text[/(\d+)/,1]} ] }

        when (text = ss.scan(/call\d+formal@[a-zA-Z_.$\#'`~^\\?][\w.$\#'`~^\\?]*@\d*_si_\d+/))
        ...
        when (text = ss.scan(/T@[a-zA-Z_.$\#'`~^\\?][\w.$\#'`~^\\?]*!val!\d+/))
           @rex_tokens.push action { [:WEIRD, nil] }

        when (text = ss.scan(/k!\d+/))
